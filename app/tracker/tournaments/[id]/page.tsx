 ...
 */
  ...
  const handleRoundOnePairings = async (client, tournamentId, round) => {
    ...
    while (remainingPlayers.length > 1) {
      ...
      matches.push({
        ...
        match_order: matches.length + 1, // Set initial order
      });
      // Remove players (remove higher index first to avoid shifting issues)
      if (index1 > index2) {
  ...
  const assignBye = async (client, tournamentId, round, playerId) => {
    try {
      ...
      const { error: byeError } = await client.from("byes").insert({
        tournament_id: tournamentId,
        round_number: round,
