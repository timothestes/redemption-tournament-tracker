...
| end-battle-races-chooser | Attacker's ↩End during the open soul chooser killed it and awarded no soul (score stayed 0-0 after a won battle) | The ↩End buttons are REMOVED entirely (Tim's call): the attacker walks away by progressing the phase — verified live that set_phase/end_turn auto-return the battle from BOTH 'active' and 'awaiting-soul', so the anti-stall hatch survives without a button one finger-width from ⚑Win. Chooser keeps its own Decline / no-souls-left exits (the defender can't phase-progress) |
