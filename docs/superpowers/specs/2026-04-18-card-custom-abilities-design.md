...
All five token cards exist in `lib/cards/generated/cardData.ts`, so no data import is needed.
...
- Reserve section tracking. The decklist Reserve section remains informational. Right-click spawn does not consume or count against Reserve supply.
...
`spawnTokenInState(state, source, ability)` is a pure function:

1. Resolve the token card via `findCard(ability.tokenName)` from `lib/cards/lookup.ts`. If missing, return `state` unchanged (log once).
2. Compute target zone:
...
3. For `n = ability.count ?? 1` iterations, build a `GameCard` with:
   - Fresh `instanceId`.
   - `ownerId: source.ownerId`.
   - `isToken: true`.
   - `cardName`, `cardSet`, `cardImgFile`, `cardType`, `brigade`, `strength`, `toughness`, `alignment`, `identifier`, `specialAbility`, `reference` copied from the token's `CardData`.
4. Append the new instances to the target zone.
...
**New server reducer** (`spacetimedb/src/index.ts`), following the `spacetimedb.reducer(params, fn)` export form used by the rest of the module:
...
export const execute_card_ability = spacetimedb.reducer(
  ...
  (ctx, { gameId, cardInstanceId, abilityIndex }) => {
    ...
    switch (ability.type) {
      ...
      case 'custom':           throw new SenderError(`Custom ability '${ability.reducerName}' not implemented`);
    }
  }
);
```

`spawnTokenImpl(ctx, source, ability, player)` is a module-local helper (not its own reducer — only one top-level reducer is needed):

1. Resolve token `CardData` from the module's card-data source for `ability.tokenName`. If missing, `throw new SenderError('Unknown token')`.
2. Compute target zone: `source.zone` if it is a play zone (`territory`, `battle`, `land-of-bondage` — match the strings used elsewhere in the module); otherwise `ability.defaultZone ?? 'territory'`.
3. For `n = ability.count ?? 1` iterations, compute `maxIdx` for the target (zone, ownerId) — same pattern as `move_card` — and insert:
   ```ts
...
4. Append a `logAction(ctx, gameId, player.id, 'SPAWN_TOKEN', ...)` entry so the game log shows the event.
...
**Client call** (object syntax, not positional):
...
- Unknown `tokenName` in the registry (fat-fingered string): client and server both log once and no-op. Caught in the registry unit test.
- Unknown `cardIdentifier` in the registry (card referenced doesn't exist): caught in the registry unit test.
- Source card not owned by sender (multiplayer): server rejects with an error; client shows the rejection the same way existing reducers do.
- Source card destroyed mid-dispatch: lookup returns undefined; both sides no-op.
- Corrupt `abilityIndex` (out of range): both sides no-op.
...
- `custom` — escape hatch for one-off abilities that don't fit a shared pattern. Client calls `conn.reducers[ability.reducerName](sourceInstanceId)`; server writes a dedicated reducer for that ability.

None of these require changes to the dispatch layer, the registry shape, or the menu integration; they only add reducer branches.
