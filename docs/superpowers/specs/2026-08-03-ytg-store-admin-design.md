...
**SKU backfill (one-time action, then rarely needed).** For every confirmed mapping (DB statuses `auto_matched`/`manual` — "matched" shorthand elsewhere in this spec means these two) whose product has no SKU: write `cardSku(card)` via `productVariantsBulkUpdate` and `custom.rtt_card_key` via `metafieldsSet`. Deliberately **not** `productSet` — avoids its price-zeroing and media list-field invariants entirely.
