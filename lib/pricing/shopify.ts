...
export async function fetchProductInventory(
  token: string,
  productIds: string[]
): Promise<Map<string, { variantId: string; inventory: number; tracked: boolean; continuesSelling: boolean }>> {
  const result = new Map<string, { variantId: string; inventory: number; tracked: boolean; continuesSelling: boolean }>();
    ...
    for (const product of data.products || []) {
      ...
      if (variant) {
        result.set(pid, {
          ...
          continuesSelling: variant.inventory_policy === 'continue',
        });
      }
    }
  }
