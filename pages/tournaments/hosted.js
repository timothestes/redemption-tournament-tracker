...
export default function HostedTournaments() {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      if (user && user.id) {
        setLoading(true);
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/tournaments?host_id=${user.id}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/tournaments/${deleteConfirmationId}`, { method: 'DELETE' });
      const data = await res.json();
      if (res.ok) {
  ...
  if (!user) {
    return (
      <div className={styles.container}>
        ...
        <h1 className={styles.title}>Please log in to view your hosted tournaments</h1>
        <Link href="/login">
  ...
  return (
    <div className={styles.container}>
      ...
      <h1 className={styles.title}>Your Hosted Tournaments</h1>
      ...
      {loading ? (
            ...
            <div className={styles.fadeOverlay}>
              <div className={styles.fadeModal}>
                ...
                <div className={styles.modalActions}>
                  <button className={`${styles.button} ${styles.deleteButton}`} onClick={handleConfirmDelete}>Delete</button>
                  <button className={`${styles.button} ${styles.neutralButton}`} onClick={handleCancelDelete}>Cancel</button>
                </div>
              </div>
            </div>
          )}
        ...
        <p className={styles.noTournaments}>No hosted tournaments found.</p>
      )}
    </div>
  );
}
