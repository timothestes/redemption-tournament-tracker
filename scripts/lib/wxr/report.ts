...
export interface PostReport {
  ...
  stats: MarkdownStats & {
    droppedBlocks: Record<string, number>; unknownRefs: string[]; removedIframes: string[];
    /** Referenced site files that are absent from the local backup. */
    missingMedia: string[];
    /** Referenced site files present in the backup whose upload failed, so the post keeps the old URL. */
    unmirroredMedia: string[];
  };
  error?: string;
}
export interface Report {
  ...
  /** Posts deliberately not imported (today: an empty WordPress title). Not failures. */
  skipped: { slug: string; wpId: string; reason: string }[];
  posts: PostReport[];
}
...
const problem = (p: PostReport) =>
  ...
  p.stats.removedIframes.length > 0 || p.stats.missingMedia.length > 0 || p.stats.unmirroredMedia.length > 0 ||
  Object.keys(p.stats.droppedBlocks).length > 0;
...
export function writeReport(dir: string, report: Report): void {
  ...
  lines.push("## Skipped posts", "", "| slug | wpId | reason |", "|---|---|---|");
  for (const s of report.skipped) lines.push(`| ${cell(s.slug)} | ${cell(s.wpId)} | ${cell(s.reason)} |`);
  lines.push("");
  ...
  for (const p of report.posts.filter(problem)) {
    ...
    if (p.stats.unmirroredMedia.length) issues.push(`unmirrored media: ${p.stats.unmirroredMedia.join(" ")}`);
