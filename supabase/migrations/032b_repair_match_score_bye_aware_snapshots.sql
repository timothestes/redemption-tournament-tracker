-- Fix: the chronological per-match snapshot rewrite in step 7 of
-- repair_match_score must include bye match-point contributions, otherwise
-- matches.player1_match_points/player2_match_points snapshots will be short
-- by 3 per bye for any participant who received one.
--
-- Approach: walk a unified chronological stream of (matches + byes) for each
-- affected participant. Byes (no match row) advance the cumulative match_points
-- by 3 but do not write a snapshot. Matches advance match_points by the
-- per-result value AND write the resulting cumulative to the match row.
...
BEGIN
  ...
  -- 6. Recompute participants.match_points / differential from history (matches + byes).
  ...
  -- 7. Chronological per-match snapshot rewrite, NOW BYE-AWARE.
  --    For each affected participant, walk matches AND byes in (round, match_order)
  --    order. Byes contribute 3 MP but no snapshot row to write. Matches advance
  --    cumulative MP/diff and rewrite the snapshot on the match row.
  FOR v_participant IN
